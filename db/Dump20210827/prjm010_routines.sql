...
BEGIN
   ...
   -- Declare variables to hold diagnostics area information
    ...
    IF par_date_fim IS NOT NULL AND par_date_fim != '' THEN
		SET @sql = CONCAT(@sql, ' AND PRJ027.daydate <= "', par_date_fim, '"');
	END IF;
    ...
    IF par_date_fim IS NOT NULL AND par_date_fim != '' THEN
		SET @sql = CONCAT(@sql, ' AND PRJM027.daydate <= "', par_date_fim, '"');
	END IF;
...
BEGIN
    ...
    SET @sql = CONCAT(@sql, ' LEFT JOIN PRJM010018 PRJM018 ON PRJM018.fireexting_id = PRJM019.fireexting_id ');
	SET @sql = CONCAT(@sql, ' INNER JOIN PRJM010029 PRJM029 ON PRJM029.local_id = PRJM018.local_id');
    SET @sql = CONCAT(@sql, ' INNER JOIN PRJM010032 PRJM032 ON PRJM032.location_id = PRJM018.location_id ');
...
BEGIN
    ...
    INSERT INTO PRJM010021
		(purifier_id,daydate,serialnumber, nextmanager)
		VALUES(par_purifier_id,par_daydate,par_serialnumber,par_nextmanager);
...
BEGIN
    ...
    IF par_daydate IS NOT NULL AND par_daydate != '' THEN
        ...
        SET @sql = CONCAT(@sql, ' OR PRJ021.nextmanager >= "', par_daydate, '"))');
	END IF;
	
	IF par_date_fim IS NOT NULL AND par_date_fim != '' THEN
		SET @sql = CONCAT(@sql, ' AND ((PRJ021.daydate <= "',par_date_fim, '"');
        SET @sql = CONCAT(@sql, ' OR PRJ021.nextmanager <= "', par_date_fim, '"))');
	END IF;
	...
	IF par_daydate IS NOT NULL AND par_daydate != '' THEN
        ...
        SET @sql = CONCAT(@sql, ' OR PRJM021.nextmanager >= "', par_daydate, '"))');
	END IF;
	
	IF par_date_fim IS NOT NULL AND par_date_fim != '' THEN
		SET @sql = CONCAT(@sql, ' AND ((PRJM021.daydate <= "',par_date_fim, '"');
        SET @sql = CONCAT(@sql, ' OR PRJM021.nextmanager <= "', par_date_fim, '"))');
	END IF;
...
BEGIN
	...
	SET @sql = concat(@sql,'	( person_id, daydate, serialnumber, location_id, local_id) ');
	SET @sql = concat(@sql,'	VALUES(',par_person_id,',"', par_daydate,'","', par_serialnumber,'",', par_location_id,',', par_local_id);
...
BEGIN
    ...
    IF par_daydate IS NOT NULL THEN
        ...
        SET @sql = CONCAT(@sql, ' INNER JOIN PRJM010010 PRJ010 USING(person_id) ');
		
        SET @sql = CONCAT(@sql, ' WHERE PRJ014.situation = 0 ');
        
        IF par_name_person IS NOT NULL AND par_name_person != '' THEN
			SET @sql = CONCAT(@sql, ' AND PRJ010.name_person LIKE "%', par_name_person, '%"' );
		END IF;
...
-- Dump completed on 2021-08-27 10:14:18
