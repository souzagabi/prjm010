...
BEGIN
    ...
    INSERT INTO PRJM010016
		(user_id,person_id,daydate,dayhour,material,qtde,packing,receiver,deliveryman)
		VALUES(par_user_id,par_person_id,par_daydate,par_dayhour,par_material,par_qtde,par_packing,par_receiver,par_deliveryman);
...
BEGIN
	...
	END IF;
    IF par_daydate IS NOT NULL THEN
		SET @sql = CONCAT('SELECT *, (SELECT count(material_id) FROM PRJM010016) / ', par_limit, ' AS pgs');
		SET @sql = CONCAT(@sql, ' FROM (SELECT * FROM PRJM010016 PRJM016 LIMIT ', par_start, ', ', par_limit, ' ) AS PRJM016 ');
		SET @sql = CONCAT(@sql, ' INNER JOIN PRJM010010 PRJM010 USING(person_id) ');
		SET @sql = CONCAT(@sql, ' WHERE PRJM016.material_id IS NOT NULL ');
...
BEGIN
    ...
    SET @sql = CONCAT(@sql, ' INNER JOIN PRJM010010 PRJM010 USING(person_id) ');
	SET @sql = CONCAT(@sql, ' WHERE PRJM016.material_id = "', par_material_id, '"');
